...
/*1. List all the student details studying in 
fourth semester ‘C’ section.
*/
select * from student
where usn in(
select c.usn from class c,semsec s
where c.ssid = s.ssid and sem =4 and sec ='c'
);
...
/*2.2. Compute the total number of male and female students
in each semester and in each section
*/
select count(gender),sem,sec,gender from 

student s, semsec se,class c
where s.usn = c.usn and c.ssid = se.ssid
group by gender,sem,sec;





/*3. Create a view of Test1 marks of student 
USN ‘1BI15CS101’ in all subjects.
*/
...
(case
WHEN IA.FINALIA >= 17 AND IA.FINALIA <20 THEN 'OUTSTANDING' 
WHEN IA.FINALIA > 12 AND IA.FINALIA <16 THEN 'AVERAGE' 
ELSE 'WEAK' 
END
) AS CAT 
